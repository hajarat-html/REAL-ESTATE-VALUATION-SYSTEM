...
if df is None:
    st.error("🚨 Error: Dataset not found. Please ensure 'synthetic_real_estate_dataset.csv' or 'nigeria_houses_data.csv' is in the same directory.")
            ...
            <div class="hero-overlay">
                ...
                <div style="display: flex; gap: 1rem; flex-wrap: wrap; justify-content: center;">
                    <a href="#" onclick="window.parent.document.querySelector('.st-emotion-cache-1pxazr7 button[data-testid=\'stOptionMenu-Prediction\']').click(); return false;" class="cta-button cta-primary">
                        GET PRICE PREDICTION <span class="icon">➜</span>
                    </a>
                </div>
            </div>
        </div>
    ...
    if not data.empty:
        ...
        with st.expander("House Price by Property Type"):
            ...
            fig_house_type = px.box(data, x='title', y='price', color='location_name',
                                    ...
                                    labels={'title': 'Property Type', 'price': 'Price (₦)'})
...
# Prediction Page
    ...
    if not data.empty and features_for_model:
        ...
        with col3:
            ...
            input_monthly_rent = st.number_input("Monthly Rent (₦)", min_value=int(data['Monthly_Rent'].min()), max_value=int(data['Monthly_Rent'].max()), value=int(data['Monthly_Rent'].mean()))
        ...
        cols_summary[2].metric("Monthly Rent (₦)", f"{input_monthly_rent:,.2f}")
        st.markdown("---")
        ...
        if st.button("Predict Price"):
            if model_pipeline and features_for_model:
                ...
                try:
                    ...
                    st.success(f"The predicted price of the property is approximately ₦{prediction[0]:,.2f}")

                    st.download_button(
                        label="Download Prediction Result",
                        data=f"Predicted Price: ₦{prediction[0]:,.2f}\n"
    ...
    if not data.empty and 'Latitude' in data.columns and 'Longitude' in data.columns and 'location_name' in data.columns:
        ...
        for idx, row in data.iterrows():
            folium.CircleMarker(
                ...
                popup=f"Price: ₦{row['price']:,.2f}<br>Location: {row['location_name']}<br>Size: {row['Size_sqft']} sqft",
                color='blue',
                fill=True,
                fill_color='blue',
                fill_opacity=0.6
        ...
        fig_location_price_geo = px.bar(avg_price_location, x='location_name', y='price',
                                     ...
                                     labels={'location_name': 'Location', 'price': 'Average Price (₦)'},
